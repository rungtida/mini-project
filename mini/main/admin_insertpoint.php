  ...
  <?php echo "<h4 align = right>";
    ...
    <div class="send-message">
      <div class="container">
        <div class="row">
          ...
          <div class="col-md-8">
            <div class="contact-form">
              <form  action="admin_insertpoint.php" method="post" name="formUserData" enctype="multipart/form-data">
                <div class="row">
                  <div class="col-sm-12">
                    <fieldset>
                      
                      <input name="adminname" type="hidden" class="form-control"  placeholder='<?php echo $_SESSION["username"]; ?>' readonly>
                    </fieldset>
                  </div>

                  <div class="col-sm-8">
                    <fieldset>
                      <input name="pname" type="text" class="form-control"  placeholder="กรอกชื่อรายการ" required="">
                    </fieldset>
                  </div>

                  <div class="col-sm-8">
                    <fieldset>
                        Pic<input name="fileUpload" type="file" class="form-control" required>
                    </fieldset>
                  </div>
                  ...
                  <div class="col-sm-6">
                    <fieldset>
                      <input name="ppoint" type="text" class="form-control"  placeholder="กรอกจำนวนพ้อยท์" required="">
                    </fieldset>
                  </div>
        ...
        @$sql2 = "SELECT MAX(pid) AS pid FROM usepoint";
        @$query = mysqli_query($conn,$sql2);
        @$rs = mysqli_fetch_array($query);
        if($rs['pid'] !=""){
            $sub = substr($rs['pid'],-3)+1;      
            $pid = sprintf('P%03d', $sub);
        }else{
            $pid = "P001";
      }
      ...
      move_uploaded_file($_FILES["fileUpload"]["tmp_name"], $filepath);
        ...
        @$sql1 = "INSERT INTO usepoint(pid,pname,ppic,ppoint) 
        VALUES('$pid','$pname','$filepath','$ppoint')";
              // echo $sql1."<br>";
              echo "<center><br>";
              echo "<font color='white'>";
              if (mysqli_query($conn, $sql1)) {
                
                echo "<script>alert('บันทึกข้อมูลเรียบร้อย');window.location='admin_insertpoint.php';</script>";
              } else {
