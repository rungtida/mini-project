...
<body>
    ...
    if (isset($_POST['save'])) {
        ...
        @$sql2 = "SELECT MAX(qid) AS qid FROM question";
    @$query = mysqli_query($conn,$sql2);
    @$rs = mysqli_fetch_array($query);
    if($rs['qid'] !=""){
        $sub = substr($rs['qid'],3)+1;      
        $qid = sprintf('Q%03.0f', $sub);
    }else{
        $qid = "Q001";
    }
    
        
        @$sql1 = "INSERT INTO question(qid, name, subject, email, detail, date, status) 
        VALUES('$qid','$name','$subject','$email','$message','$date','$status')";
                // echo $sql1."<br>";
                echo "<center><br>";
